...
char *
url_data_merge(url_data_t *data)
{
    int slen = 0;
    char* new_url = NULL;

    slen = strlen(data->href);
    new_url = (char *)malloc(slen + 10); 
    if(!new_url)
        return NULL;
    memset(new_url,0x00,slen+10);
    strcat(new_url,data->protocol);
    strcat(new_url,"://");
    slen = strlen(data->auth);
    if(slen)
    {
        strcat(new_url,data->auth);
        strcat(new_url,"@");
    }
    strcat(new_url,data->host);
    slen = strlen(data->port);
    if(slen)
    {
        strcat(new_url,data->port);
    }
    slen = strlen(data->path);
    if(slen)
    {
        strcat(new_url,data->path);
    }
